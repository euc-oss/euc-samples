...
# Copyright 2022 Omnissa
#
# revision 4 (December 11, 2024)
#
# macOS Migrator
...
migratorlog="/var/log/ws1_migrator.log"
depnotifylog="/private/var/tmp/depnotify.log"
depnotifypath="/Applications/Utilities/DEPNotify.app"
hubpath="https://packages.omnissa.com/wsone/WorkspaceONEIntelligentHub.pkg"
migratorpath="/Library/Application Support/WS1/migrator.sh"
resourcesdir="/Library/Application Support/WS1/MigratorResources"
ldpath="/Users/mzaske/Library/CloudStorage/OneDrive-Omnissa/Documents/Script Testing/com.ws1.migrator.plist"
ldidentifier="com.ws1.migrator"
depProfileName=""
currentOS=$(sw_vers -productVersion)
currentUser=$(stat -f%Su /dev/console)
currentUID=$(id -u "$currentUser")
#customization Scripts
predepnotifyScript="/Library/Application Support/WS1/MigratorResources/predepnotify.sh"
premigrationScript="/Library/Application Support/WS1/MigratorResources/premigration.sh"
midmigrationScript="/Library/Application Support/WS1/MigratorResources/midmigration.sh"
postmigrationScript="/Library/Application Support/WS1/MigratorResources/postmigration.sh"
...
verifyArgs() {
  ...
  if [[ "$removalScript" == "" ]]; then removalScript="/Library/Application Support/WS1/MigratorResources/removemdm.sh"; fi
  if [[ "$enrollmentProfilePath" == "" ]]; then enrollmentProfilePath="/Library/Application Support/WS1/MigratorResources/enroll.mobileconfig"; fi
  ...
  if [[ ! -z "$baseurl" ]]; then hubpath="$baseurl/DeviceServices/resources/WorkspaceONEIntelligentHub.pkg"; fi
  ...
  log "baseurl: $baseurl"
  log "dest_auth: $dest_auth"
  log "dest_token: $dest_token"
  log "groupid: $groupid"
...
init_registration() {
  ...
  if [[ "$promptType" = "username" ]]; then
    ...
    /bin/launchctl asuser "$currentUID" sudo -u $currentUser defaults write menu.nomad.DEPNotify registrationMainTitle "Enter your email address"
...
registerDeviceWS1() {
  ...
  registration='{"PlatformId":10, "MessageType":0, "ToEmailAddress":"noreply@omnissa.com", "Ownership":"C", "LocationGroupId":"'$locationGroupID'", "SerialNumber":"'$serial'", "FriendlyName":"'$username' '$deviceType'"}'
...
hubInstall() {
  ...
  if [[ "$registrationType" = "none" ]]; then
    ...
    if [[ ! "$huburl" = "" ]]; then
      hubpath="$huburl/DeviceServices/resources/WorkspaceONEIntelligentHub.pkg"
    fi
  fi
  #download hub
...
log "Using registration mode: $registrationType"
