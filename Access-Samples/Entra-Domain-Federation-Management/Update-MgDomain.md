...
1. Validates required inputs (tenant ID, domain, authentication type).
...
- Module loading is scoped to the minimum required Graph module to reduce startup latency.
- `DomainId` is accepted as a backward-compatible alias for `Domain`.
